...
function register!(rr::Router, handler; path=nothing, methods=nothing,
            host=nothing, headers=[])
    route = Route(handler, path, methods, host, headers)
...
function match_handler(rr::Router, req)
    ...
    for route in rr.routes
        if matchroute(route, req)
            global Vars = Dict{String, Any}()
            for m in route.matchers
                if typeof(m) == Regexpmatcher
                    merge!(Vars, setvarnames(m, req))
                end
            end
